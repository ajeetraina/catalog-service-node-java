...
if exist %MAVEN_PROJECTBASEDIR%\.mvn\wrapper\maven-wrapper.jar (
    ...
    powershell -Command "&{"^[System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12; ^"^}; $wc = New-Object System.Net.WebClient; $wc.DownloadFile('%WRAPPER_URL%', '%MAVEN_PROJECTBASEDIR%\.mvn\wrapper\maven-wrapper.jar')"
    if "%MVNW_VERBOSE%" == "true" (
...
IF NOT "%WRAPPER_SHA_256_SUM%"=="" (
    powershell -Command "&{"^[System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12; ^"^}; $hash = Get-FileHash '%MAVEN_PROJECTBASEDIR%\.mvn\wrapper\maven-wrapper.jar' -Algorithm SHA256; if ($hash.Hash -ne '%WRAPPER_SHA_256_SUM%') { Write-Error 'Error: Failed to validate Maven wrapper SHA-256, your Maven wrapper might be compromised.' -ErrorAction Stop; exit 1; }"
